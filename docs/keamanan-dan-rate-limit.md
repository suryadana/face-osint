...
Catatan auto-stop: karena butuh konsensus (≥2 gambar berbeda), akun dengan hit kuat hanya di 1 gambar (mis. profile pic saja) **tidak** memicu auto-stop — sampling lanjut ke post berikutnya sampai konsensus tercapai atau `post_urls` habis, jadi volume request untuk akun semacam itu tidak berkurang hanya karena skor tinggi di satu gambar.
